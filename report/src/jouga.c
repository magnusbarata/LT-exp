...
void alg_collect_GreenStart()
{
  ...
  mov_func(-50, 0, -5);*/
  mov_func(-50, 310, -145); // Prev:(-50, 320, -150), Shove the ball on the left

  // 2個目
  mov_func(50, 0, 510);
  arm_func(12, -1);
  steering(-1, 360 * 0.6); // Aiming ball on left
  mov_func(-50, 0, -695); // Prev:(-50, 0, -680), Forward until first red ball
  /*while (ecrobot_get_touch_sensor(Rtouch) == 0) mov_func(-50, 0, -5);
  mov_func(-50, 0, -10);*/
  ...
  steering(-50, 360 * 1.5);
  mov_func(-50, 500, 750);
  mov_func(-100, 0, 950); // All power straight to base
}
