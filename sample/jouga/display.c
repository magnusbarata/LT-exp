/*
 * /nxtOSEK/lejos_nxj/src/nxtvm/platform/nxt/display.cを修正した物
 * 
 * graphics.cから使えるようにstaticをいくつか外した
 *	変数等が外部から見えるようになっただけで使い勝手は変らない
 * 
 * 文字列の反転表示と自動スクロール機能を加えた
 *	最終行で改行文字をdisplay_stringで出力する時の挙動が異なる
 *	display_clearでカーソル位置が左上に戻るのが異なる
 * 
 * このファイルをここのMakefileで指定すれば
 * 元のファイルは使われなくなる
 */
...
U8 (*display_buffer)[DISPLAY_WIDTH] = display_array.display;
...
const U8 font[N_CHARS][FONT_WIDTH] = {
...
static void
display_char_sub(int c, U8 back)
{
  int i;
  U8 *b;
  const U8 *f;
...
display_char(int c)
{
  display_char_sub(c, 0x00);
}

void
display_char_inverted(int c)
{
  display_char_sub(c, 0xff);
}

static void
display_string_sub(const char *str, U8 back)
{
  while (*str) {
    if (*str != '\n') {
      display_char_sub(*str, back);
      display_x++;
    } else {
      display_x = 0;
      display_y++;
#ifdef SCROLL_DISPLAY
      if (display_y == DISPLAY_DEPTH) {
        long *p = (long *)display_buffer;
        long *q = (long *)&display_buffer[1];
        int l = DISPLAY_WIDTH * DISPLAY_DEPTH / sizeof(long);
...
{
  display_string_sub(str, 0x00);
}

void
display_string_inverted(const char *str)
{
  display_string_sub(str, 0xff);
}
...
display_test(void)
{
  ...
  display_clear(0);
  display_string("1st line\n");
  display_string(" 2nd line\n");
  display_string("  3rd line\n");
  display_update();
  go_next();
  display_string_inverted("4th line\n");
  display_string_inverted(" 5th line\n");
  display_string_inverted("  6th line\n");
  display_update();
  go_next();
  display_string("7th line\n");
  display_update();
  go_next();
  display_string(" 8th line\n");
  display_update();
  go_next();
  display_string("  9th line");
  display_update();
  go_next();
  display_string("\n   last line");
  display_update();
